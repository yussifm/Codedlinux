...
static inline void apple_nvme_handle_cqe(struct apple_nvme_queue *nvmeq, u16 idx)
{
	...
	apple_nvmmu_inval(nvmeq, command_id);
	...
	req = blk_mq_tag_to_rq(apple_nvme_queue_tagset(nvmeq), command_id);
	if (unlikely(!req)) {
...
{
	struct apple_nvme_queue *nvmeq = data;
...
static irqreturn_t apple_nvme_irq_check(int irq, void *data)
{
	struct apple_nvme_queue *nvmeq = data;

	if (apple_nvme_cqe_pending(nvmeq))
...
static int apple_adapter_delete_queue(struct apple_nvme_dev *dev, u8 opcode, u16 id)
{
	struct nvme_command c = { };
...
static int apple_adapter_alloc_cq(struct apple_nvme_dev *dev, u16 qid,
		struct apple_nvme_queue *nvmeq, s16 vector)
{
	struct nvme_command c = { };
...
static int apple_adapter_alloc_sq(struct apple_nvme_dev *dev, u16 qid,
						struct apple_nvme_queue *nvmeq)
{
	struct nvme_command c = { };
...
static int apple_adapter_delete_cq(struct apple_nvme_dev *dev, u16 cqid)
{
	return apple_adapter_delete_queue(dev, nvme_admin_delete_cq, cqid);
}

static int apple_adapter_delete_sq(struct apple_nvme_dev *dev, u16 sqid)
{
	return apple_adapter_delete_queue(dev, nvme_admin_delete_sq, sqid);
}
...
static void apple_nvme_free_queues(struct apple_nvme_dev *dev, int lowest)
{
	if (WARN_ON(lowest > 1))
		return;
...
static void apple_nvme_queue_free_irq(struct apple_nvme_queue *nvmeq)
{
...
static int apple_nvme_suspend_queue(struct apple_nvme_queue *nvmeq)
{
	...
	apple_nvme_queue_free_irq(nvmeq);
	return 0;
}

static void apple_nvme_suspend_io_queues(struct apple_nvme_dev *dev)
{
...
static int apple_nvme_alloc_sq_cmds(struct apple_nvme_dev *dev, struct apple_nvme_queue *nvmeq,
				int qid)
{
...
static int apple_nvme_alloc_queue(struct apple_nvme_dev *dev, int qid, int depth)
{
	struct apple_nvme_queue *nvmeq;

	if (depth > APPLE_ANS2_QUEUE_DEPTH)
		return -EINVAL;
	...
	if (qid == 0) {
		lo_hi_writeq(nvmeq->ans2_tcb_dma_addr,
			     dev->nvme_mmio + APPLE_NVMMU_BASE_ASQ);
		nvmeq->ans2_q_db = dev->nvme_mmio + APPLE_ANS2_LINEAR_ASQ_DB;
	} else {
		lo_hi_writeq(nvmeq->ans2_tcb_dma_addr,
			     dev->nvme_mmio + APPLE_NVMMU_BASE_IOSQ);
		nvmeq->ans2_q_db = dev->nvme_mmio + APPLE_ANS2_LINEAR_IOSQ_DB;
	}

	if (apple_nvme_alloc_sq_cmds(dev, nvmeq, qid))
		goto free_ans2;
 ...
 free_ans2:
	if (nvmeq->ans2_tcb_ptr) {
		...
		if (qid == 0)
			lo_hi_writeq(0, dev->nvme_mmio + APPLE_NVMMU_BASE_ASQ);
		else
			lo_hi_writeq(0, dev->nvme_mmio + APPLE_NVMMU_BASE_IOSQ);
	}
 free_cqdma:
...
static int apple_nvme_queue_request_irq(struct apple_nvme_queue *nvmeq)
{
	irq_handler_t handler, thread_fn;

	if (use_threaded_interrupts) {
		handler = apple_nvme_irq_check;
		thread_fn = apple_nvme_irq;
	} else {
		handler = apple_nvme_irq;
		thread_fn = NULL;
	}
...
static void apple_nvme_init_queue(struct apple_nvme_queue *nvmeq, u16 qid)
{
	struct apple_nvme_dev *dev = nvmeq->dev;
...
static int apple_nvme_create_queue(struct apple_nvme_queue *nvmeq, int qid)
{
	struct apple_nvme_dev *dev = nvmeq->dev;
	int result;
	...
	result = apple_adapter_alloc_cq(dev, qid, nvmeq, 0);
	if (result)
		return result;

	result = apple_adapter_alloc_sq(dev, qid, nvmeq);
	if (result < 0)
		return result;
	if (result)
		goto release_cq;

	apple_nvme_init_queue(nvmeq, qid);

	result = apple_nvme_queue_request_irq(nvmeq);
	if (result < 0)
		goto release_sq;
...
release_sq:
	dev->online_queues--;
	apple_adapter_delete_sq(dev, qid);
release_cq:
	apple_adapter_delete_cq(dev, qid);
	return result;
}
...
static int apple_nvme_configure_admin_queue(struct apple_nvme_dev *dev)
{
	...
	dev->subsystem = readl(dev->nvme_mmio + NVME_REG_VS) >= NVME_VS(1, 1, 0) ?
				NVME_CAP_NSSRC(dev->ctrl.cap) : 0;

	if (dev->subsystem &&
	    (readl(dev->nvme_mmio + NVME_REG_CSTS) & NVME_CSTS_NSSRO))
		writel(NVME_CSTS_NSSRO, dev->nvme_mmio + NVME_REG_CSTS);

	result = nvme_disable_ctrl(&dev->ctrl);
	if (result < 0)
		return result;

	result = apple_nvme_alloc_queue(dev, 0, NVME_AQ_DEPTH);
	if (result)
		return result;
	...
	apple_nvme_init_queue(dev->adminq, 0);
	result = apple_nvme_queue_request_irq(dev->adminq);
	if (result) {
		dev->online_queues--;
		return result;
	}
...
static int apple_nvme_create_io_queues(struct apple_nvme_dev *dev)
{
	int ret = 0;

	if (apple_nvme_alloc_queue(dev, 1, dev->q_depth))
		return -ENOMEM;

	ret = apple_nvme_create_queue(dev->ioq, 1);
	if (ret)
		return ret;
...
static int apple_nvme_setup_io_queues(struct apple_nvme_dev *dev)
{
	...
	result = apple_nvme_create_io_queues(dev);
...
static void apple_nvme_dev_add(struct apple_nvme_dev *dev)
{
	...
	if (!dev->ctrl.tagset) {
		...
		WARN_ON(dev->online_queues > 2);
		/* Free previously allocated queues that are no longer usable */
		apple_nvme_free_queues(dev, dev->online_queues);
	}
}
...
static void apple_nvme_dev_disable(struct apple_nvme_dev *dev, bool shutdown)
{
	...
	apple_nvme_suspend_io_queues(dev);
...
static struct apple_nvme_dev *apple_nvme_dev_alloc(struct device *parent)
{
	...
	dev->dev = get_device(parent);
        dev->adminq = kmalloc_node(sizeof(*dev->adminq), GFP_KERNEL, node);
        if (!dev->adminq)
                goto free_dev;

	dev->ioq = kmalloc_node(sizeof(*dev->ioq), GFP_KERNEL, node);
        if (!dev->ioq)
                goto free_adminq;
...
static int apple_nvme_probe(struct platform_device *pdev)
{
	...
	dev->sart_mmio = devm_platform_ioremap_resource_byname(pdev, "sart");
	if (IS_ERR(dev->sart_mmio)) {
		result = PTR_ERR(dev->sart_mmio);
		goto out_unmap;
	}
	...
	dev->rtk = apple_rtkit_init(dev->dev, dev, res, "ans-mbox",
				    &sart_rtkit_ops);
	if (!dev->rtk) {
	...
	ret = apple_rtkit_boot_wait(dev->rtk);
	if (ret) {
