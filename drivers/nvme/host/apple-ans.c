...
{
	return apple_adapter_delete_queue(dev, nvme_admin_delete_cq, 1);
}

static int apple_adapter_delete_sq(struct apple_nvme_dev *dev)
{
	return apple_adapter_delete_queue(dev, nvme_admin_delete_sq, 1);
}
...
static int apple_nvme_alloc_queue(struct apple_nvme_dev *dev, int qid, int depth)
{
	...
	nvmeq->sqes = nvmeq->is_adminq ? NVME_ADM_SQES : NVME_NVM_IOSQES;
	nvmeq->q_depth = depth;
...
static int apple_nvme_create_io_queue(struct apple_nvme_dev *dev)
{
	...
	apple_nvme_init_queue(dev->ioq, 1);
	ret = apple_nvme_queue_request_irq(dev->ioq);
	if (ret < 0)
		goto release_sq;
