			       ...
			       struct apple_mbox_msg *msg)
{
	 ...
	 * data stored in a shared memory buffer and we must ensure we've
	 * completed all previous writes to normal memory before submitting
	 * this message.
	 */
	dma_wmb();
			       ...
			       struct apple_mbox_msg *msg)
{
	...
	/*
	 * we need a DMA barrier here since this message might be related to
	 * data stored in a shared memory buffer and we must ensure we only
	 * read normal memory after we've read this message.
	 */
	dma_rmb();
}
