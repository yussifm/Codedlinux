				     ...
				     void *buf, size_t size, size_t wsize)
{
	...
	if (size <= 4)
		...
		memcpy_fromio(buf, smc->shmem_base, size);
...
static int apple_smc_rtkit_write_key(void *cookie, smc_key key, void *buf, size_t size)
{
	...
	memcpy_toio(smc->shmem_base, buf, size);
				  ...
				  void *wbuf, size_t wsize, void *rbuf, size_t rsize)
{
	...
	memcpy_toio(smc->shmem_base, wbuf, wsize);
...
static void __iomem *apple_smc_rtkit_map(void *cookie, dma_addr_t addr, size_t len)
{
	struct apple_smc_rtkit *smc = cookie;
	struct resource res = {
		.start = addr,
	...
	return smc->sram_base + (res.start - smc->sram->start);
}
...
static void apple_smc_rtkit_recv(void *cookie, u8 endpoint, u64 message)
{
	...
	if (!smc->initialized) {
		smc->shmem_base = apple_smc_rtkit_map(smc, message, SMC_SHMEM_SIZE);
		if (IS_ERR(smc->shmem_base))
