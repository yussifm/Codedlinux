						 ...
						 u64 msg)
{
	...
	dev_info(rtk->dev, "RTKit: system endpoints successfuly initialized!");
}
					     ...
					     u8 ep, u64 msg)
{
	...
	if (rtk->ops->shmem_owner == APPLE_RTKIT_SHMEM_OWNER_RTKIT) {
		...
		if (ret) {
			dev_warn(
				rtk->dev,
				"RTKit: buffer verification failed for 0x%zx bytes at 0x%llx\n",
				size, iova);
			return;
		}
	...
	if (!buffer->buffer) {
		dev_warn(rtk->dev, "RTKit: couldn't allocate 0x%zx bytes.\n",
			 size);
		return;
	}
...
static void apple_rtkit_syslog_rx_log(struct apple_rtkit *rtk, u64 msg)
{
	...
	if (rtk->ops->shmem_owner == APPLE_RTKIT_SHMEM_OWNER_LINUX &&
		...
		dev_warn(
			rtk->dev,
			"RTKit: received syslog message but have no syslog_buffer.buffer");
		goto done;
	}
	if (rtk->ops->shmem_owner == APPLE_RTKIT_SHMEM_OWNER_RTKIT &&
		...
		dev_warn(
			rtk->dev,
			"RTKit: received syslog message but have no syslog_buffer.iomem");
		goto done;
	}
	if (idx > rtk->syslog_n_entries) {
		dev_warn(rtk->dev, "RTKit: syslog index %d out of range", idx);
		goto done;
	}
