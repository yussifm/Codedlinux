...
TRACE_EVENT(atcphy_usb3_set_mode,
	    ...
	    TP_STRUCT__entry(__field(enum atcphy_mode, mode)
				     __field(enum atcphy_mode, target_mode)
					     __field(enum phy_mode, phy_mode)
						     __field(int, submode)),

	    TP_fast_assign(__entry->mode = atcphy->mode;
			   __entry->target_mode = atcphy->target_mode;
			   __entry->phy_mode = mode;
			   __entry->submode = submode;),

	    TP_printk("mode: %s, target_mode: %s, phy_mode: %d, submode: %d",
		      show_atcphy_mode(__entry->mode),
		      show_atcphy_mode(__entry->target_mode), __entry->phy_mode,
		      __entry->submode));
...
TRACE_EVENT(
	...
	TP_STRUCT__entry(__field(enum atcphy_mode, mode) __field(
		const struct atcphy_mode_configuration *, cfg)),
...
TRACE_EVENT(atcphy_mux_set, TP_PROTO(struct typec_mux_state *state),
	    ...
	    TP_STRUCT__entry(__field(struct typec_mux_state *, state)),
...
TRACE_EVENT(atcphy_parsed_tunable,
	    ...
	    TP_STRUCT__entry(__field(const char *, name)
				     __field(struct atcphy_tunable *, tunable)),

	    TP_fast_assign(__entry->name = name; __entry->tunable = tunable;),

	    TP_printk("%s with %ld entries", __entry->name,
		      __entry->tunable->sz));
